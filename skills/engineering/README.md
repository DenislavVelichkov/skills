...
- **[implement](./implement/SKILL.md)**: Build one approved ticket or one small plan retained in the current conversation, driving `/tdd` at pre-agreed seams and reviewing the committed integrated candidate with `/code-review`.
...
- **[code-review](./code-review/SKILL.md)**: Two-axis review of the diff since a fixed point: **Standards** (does it follow the repo's coding standards, plus a Fowler smell baseline?) and **Spec** (does it faithfully implement the originating issue/spec?), run locally by default, with explicitly authorized parallel review.
